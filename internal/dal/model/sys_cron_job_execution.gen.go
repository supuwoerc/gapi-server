...
type CronJobExecution struct {
	...
	JobName     string     `gorm:"column:job_name;type:varchar(128);not null;comment:任务名称" json:"job_name"`                                            // 任务名称
	Status      string     `gorm:"column:status;type:varchar(16);not null;comment:执行状态(pending/running/success/failed/cancelled/panic)" json:"status"` // 执行状态(pending/running/success/failed/cancelled/panic)
