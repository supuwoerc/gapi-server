...
type UserRepository interface {
	...
	UpdateProfile(ctx context.Context, id int64, username, bio, avatar string) error
	UpdateCompletedTours(ctx context.Context, id int64, tours []string) error
}
...
func (s *UserService) GetMenuPermissions(ctx context.Context, roleIDs []int64) ([]string, error) {
...
func (s *UserService) GetRoutePermissions(ctx context.Context, roleIDs []int64) ([]string, error) {
...
func (s *UserService) UpdateProfile(ctx context.Context, userID int64, name, bio, avatar string) (*model.User, error) {
	if err := s.UserRepo.UpdateProfile(ctx, userID, name, bio, avatar); err != nil {
		s.Logger.Ctx(ctx).Error("update profile failed", zap.Int64("userID", userID), zap.Error(err))
		return nil, response.InternalError
	}
	user, err := s.UserRepo.FindByID(ctx, userID)
	if err != nil {
		s.Logger.Ctx(ctx).Error("find user after profile update failed", zap.Int64("userID", userID), zap.Error(err))
		return nil, response.InternalError
	}
	return user, nil
}

func (s *UserService) UpdateCompletedTours(ctx context.Context, userID int64, newTours []string) ([]string, error) {
	...
	user, err := s.UserRepo.FindByID(ctx, userID)
	if err != nil {
		s.Logger.Ctx(ctx).Error("find user failed", zap.Int64("userID", userID), zap.Error(err))
		return nil, response.InternalError
	}
	...
	if err := s.UserRepo.UpdateCompletedTours(ctx, userID, merged); err != nil {
		s.Logger.Ctx(ctx).Error("update completed tours failed", zap.Int64("userID", userID), zap.Error(err))
		return nil, response.InternalError
	}
	return merged, nil
}
